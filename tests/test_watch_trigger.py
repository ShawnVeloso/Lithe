...
# DB isolation comes from the shared `isolated_db` fixture in tests/conftest.py,
# which patches both memory.DB_PATH and config.DB_PATH.

def test_watch_trigger_rule_matching(isolated_db, tmp_path):
    ...
    rule_dir = os.path.normcase(os.path.realpath(str(tmp_path / "watched")))
...
def test_active_rule_filtering(isolated_db, tmp_path):
