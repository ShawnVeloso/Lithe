...
- [x] **U-18: read_file Tool & Honest Descriptions** — `read_file` lets the model inspect a file it located via `search_files` (40KB cap, explicit truncation marker, same path guard and 30s timeout as the mutating tools). `search_files` no longer claims to find files by their contents, and discloses its 20-result cap.
- [x] **U-17: Bounded Agent Loop** — Tools chain within a single turn (`MAX_TOOL_ROUNDS = 5`): the model can search, see the result and act on it. Mutating tools still pause for confirmation and resume into the loop with the remaining budget. Previously a hard-coded two-call chain silently discarded any second-round tool call.
